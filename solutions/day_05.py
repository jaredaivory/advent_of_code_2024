...
class Solution(SolutionClass[Tuple[List[Tuple[int, int]], List[List[int]]], int]):
...
def is_correctly_ordered(
    ordering_hash_set: Mapping[int, Set[int]], update: List[int]
) -> bool:
    current_set = set()
    for val in update[::-1]:
...
def import_from_file(path: str) -> Tuple[List[Tuple[int, int]], List[List[int]]]:
    with open(path, "r") as file:
        ...
        ordering_rules = [
            tuple([int(val) for val in tuple(line.split("|"))])
            for line in raw_ordering_rules.split("\n")
        ]
        updates = [
            [int(val) for val in line.split(",")] for line in raw_updates.split("\n")
        ]
